...
while cap.isOpened():
    ...
    if results.multi_hand_landmarks:
        if len(results.multi_hand_landmarks) == 2:
            ...
            distances1 = [
                calculate_distance(hand_landmarks1.landmark[4], hand_landmarks1.landmark[8]),
                calculate_distance(hand_landmarks1.landmark[4], hand_landmarks1.landmark[12]),
                calculate_distance(hand_landmarks1.landmark[4], hand_landmarks1.landmark[16]),
                calculate_distance(hand_landmarks1.landmark[4], hand_landmarks1.landmark[20]),
            ]
            distances2 = [
                calculate_distance(hand_landmarks2.landmark[4], hand_landmarks2.landmark[8]),
                calculate_distance(hand_landmarks2.landmark[4], hand_landmarks2.landmark[12]),
                calculate_distance(hand_landmarks2.landmark[4], hand_landmarks2.landmark[16]),
                calculate_distance(hand_landmarks2.landmark[4], hand_landmarks2.landmark[20]),
            ]

            # Calculate averages
            average_distance = 100 - ((sum(distances1) + sum(distances2)) * 100 / (len(distances1) +len(distances2)))

            cv2.putText(image, f'Avg Dist: {average_distance:.2f}%', (10,60), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 1, cv2.LINE_AA)
