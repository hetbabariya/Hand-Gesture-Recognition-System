...
# Function to classify the "G" gesture
def classify_g_gesture(landmarks1, landmarks2):
    distance_threshold = 0.15  # Adjust this threshold as needed

    # Calculate distances for the required landmark pairs
    distance_18_2 = calculate_distance(landmarks1[18], landmarks2[2])
    distance_17_5 = calculate_distance(landmarks1[17], landmarks2[5])

    # Check if the specified landmark pairs are within the threshold distance
    if distance_18_2 < distance_threshold and distance_17_5 < distance_threshold:
        return "G"
    return "Not G"
...
while cap.isOpened():
    ...
    if results.multi_hand_landmarks:
        if len(results.multi_hand_landmarks) == 2:
            ...
            # Classify the G gesture
            gesture = classify_g_gesture(hand_landmarks1.landmark, hand_landmarks2.landmark)
            cv2.putText(image, f'Gesture: {gesture}', (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 2, cv2.LINE_AA)
        else:
            cv2.putText(image, 'Gesture: Not G', (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 2, cv2.LINE_AA)
