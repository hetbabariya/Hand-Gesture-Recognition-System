...
# Function to classify the "Z" gesture
def classify_z_gesture(landmarks1, landmarks2):
    thumb = landmarks2[4]
    palm_base = landmarks2[0]

    # Points of index, middle, ring and pinki fingers
    index_finger = landmarks1[8]
    middle_finger = landmarks1[12]
    ring_finger = landmarks1[16]
    pinki_finger = landmarks1[20]
    
    # Check if the fingers are above the line between thumb and palm base
    if is_point_above_line(index_finger, thumb, palm_base) and \
       is_point_above_line(middle_finger, thumb, palm_base) and \
       is_point_above_line(ring_finger, thumb, palm_base) and \
       is_point_above_line(pinki_finger, thumb, palm_base):
        return "Not Z"
    return "Z"
...
while cap.isOpened():
    ...
    if results.multi_hand_landmarks and len(results.multi_hand_landmarks) == 2:
        ...
        # Classify the Z gesture
        gesture = classify_z_gesture(hand_landmarks1.landmark, hand_landmarks2.landmark)
